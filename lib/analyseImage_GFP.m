...
% ----------------------------------------------------------------------- 
%
function [ msra, lab, labEllipse, imgSpheroids, spotTable, imgSpots, radialProfiles ] = analyseImage_GFP(options)
...
%% LOAD THE SPOTS IMAGE
    ...
    tstart = tic;
    channelId = options.input.channelIdStroma;
    disp('ChannelId = ');disp(channelId)
    img = loadMicroscopeImageStack( options.input.imageDir, options.input.fileName, channelId, options.input.seriesId, options.input.imageMicroscopeFormat );
    tstop = toc(tstart);
    fprintf('Processing time LoadImage = %i\n', tstop);
    
    %switch options.stroma.segmentationMethod
    %    case 'heightmap'
    options.stroma.pixelSize = pixelSize;
    [imgMIPZ_gfp, imgMIPZH_gfp, lab_gfp, contour_gfp] = spheroidSegmentation2D( img, options.stroma );
    %end
    dipshow(imgMIPZ_gfp);
    dipshow(imgMIPZH_gfp);
    dipshow(contour_gfp);
