function lab = segmentHeightMap2D_noFilter( imgMIPZ, imgMIPZH, pixelSize, neighbourhoodRadius, maxRangeZ, removeBorderObjectsInZ, borderZRemoveMethod)
% -----------------------------------------------------------------------
% 
...
%
    ...
    fprintf('Process: segmentHeightMap2D_noFilter\n');
    tic;
    ...
    % Be carful with filling holes: if the background has large regions of
    % similar height and becomes identified as foreground, by filling holes
    % at this stage we remove the opportunity to remove low intensity
    % regions up front
    imgt = fillholes(img_range < maxRangeZPixels );
    
	minRadius = pixelSize(1);
    ...
    endTime = toc();
    fprintf('segmentHeightMap2D_noFilter: time duration: %s\n', num2str(endTime));
end
