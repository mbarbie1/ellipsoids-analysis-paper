...
function updateImagesGUI(fh)
    ...
    style = {'checkbox', 'text', 'text', 'edit','popupmenu','popupmenu','popupmenu'};
    w = [ 0.05, 0.2, 0.2, 0.2, 0.1, 0.1, 0.1 ];
    ...
    for k = 1:(length(y))
        ...
        string = {'', options.output.images{1,k}.label, options.output.images{1,k}.dir, options.output.images{1,k}.pattern,...
            {'tif','png'}, {'rgb','gray'}, {'uint8','uint16'} };
        call = {'Callback','Callback','ButtonDownFcn','Callback','Callback','Callback','Callback'};
...
function handler = numericParameterInput( fh, panel, y, w, x, h, parameter, label, default, override, handlerName, parameterCallback )

    style = {'text','edit','checkbox'};
    string = {label,num2str(default),override};
    ...
    handler = uicontrol(...
            ...
            'String',parameter,...
            'Position', [ x, y, w(m), h ],...
            'Callback',{@optionsNumericParameter_callback, tableCallback});
        x = x + w(m);
...
function initSettings(fh)
    ...
    for k = 1:nSettings
        ...
        switch options.settings{k}.type
            case 'numeric'
                S.options.(options.settings{1,k}.category).(options.settings{1,k}.name) = options.settings{1,k}.value;
            case 'boolean'
                S.options.(options.settings{1,k}.category).(options.settings{1,k}.name) = options.settings{1,k}.value;
            case 'choice'
                S.options.(options.settings{1,k}.category).(options.settings{1,k}.name) = options.settings{1,k}.list(options.settings{1,k}.selected);
                %disp(S.options.(options.settings{1,k}.category).(options.settings{1,k}.name))
            otherwise
...
function [ winx, winy, winw, winh, groups, buttonh, marginx ] = getWindowSpecifications(options)
    ...
    groups(j).h = 0.1;
    groups(j).position = [marginx,marginy,1-2*marginx,groups(j).h];
...
function settingsUpdate_callback(hObject, eventdata, varargin)
    ...
    switch (formType)
        case ''
            ...
            disp(S.options.settings{k}.type);
            switch (settingType)
                case 'choice'
